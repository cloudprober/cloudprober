...
// cloudprober's metrics/payload parser, so values can be anything the parser
// understands (ints, floats, strings, maps, distributions).
...
func (h typedHandler[C, S]) decodeConfig(config []byte) (C, error) {
	...
	if err := json.Unmarshal(config, &c); err != nil {
		return c, fmt.Errorf("decoding probe config: %v", err)
	}
	return c, nil
}
