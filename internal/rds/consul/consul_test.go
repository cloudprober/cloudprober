...
func TestAggregateHealthStatus(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Build a []consulCheck from the test case statuses.
			checks := make([]consulCheck, len(tt.statuses))
			for i, status := range tt.statuses {
				checks[i] = consulCheck{Status: status}
			}

			got := aggregateHealthStatus(checks)
			if got != tt.want {
				t.Errorf("aggregateHealthStatus() = %q, want %q", got, tt.want)
			}
		})
	}
}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, res := range resp.Resources {
				if res.Labels["node"] == "node1" {
					node1Labels = res.Labels
					break
				}
			}
			if len(tt.wantLabels) > 0 && node1Labels == nil {
			...
			for _, dontWantLabel := range tt.dontWantLabels {
				for _, res := range resp.Resources {
					if _, exists := res.Labels[dontWantLabel]; exists {
						t.Errorf("Expected label %s to not exist, but found it on resource %s", dontWantLabel, res.GetName())
					}
				}
			}
		})
