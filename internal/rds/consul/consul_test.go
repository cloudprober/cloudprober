...
func TestServicesListerWithAdditionalLabels(t *testing.T) {
	...
	tests := []struct {
		...
		wantLabels        map[string]map[string]string // resource name -> labels
	}{
		{
			name: "additional_labels added to all resources",
			config: &configpb.ProviderConfig{
			...
			wantResourceCount: 3, // web-1, web-2, and api-1 (db-1 is critical)
		...
		{
			name: "additional_labels with health filter",
			config: &configpb.ProviderConfig{
				Address: proto.String(serverAddr),
				Services: &configpb.ServicesConfig{
			...
			wantResourceCount: 2, // Only web-1 and web-2 are passing
			wantLabels: map[string]map[string]string{
				"web": {
					"cluster": "prod-cluster",
					"service": "web",
		...
		{
			name: "additional_labels don't override existing labels",
			config: &configpb.ProviderConfig{
				Address: proto.String(serverAddr),
				Services: &configpb.ServicesConfig{
					AdditionalLabels: map[string]string{
						"service": "should-not-override", // This should NOT override the actual service label
						"custom":  "value",
					},
				},
			},
			...
			wantLabels: map[string]map[string]string{
				"web": {
					"service": "web", // Original service label preserved
					"custom":  "value",
				},
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify labels for specific services
			for _, res := range resp.Resources {
				serviceName := res.GetName()
				if expectedLabels, ok := tt.wantLabels[serviceName]; ok {
					for key, expectedValue := range expectedLabels {
						if gotValue, exists := res.Labels[key]; !exists {
							t.Errorf("Resource %s missing label %s", serviceName, key)
						} else if gotValue != expectedValue {
							t.Errorf("Resource %s label %s = %s, want %s", serviceName, key, gotValue, expectedValue)
						}
					}
				}
			}
