...
func TestIntegrationConsulRDSWithAdditionalLabels(t *testing.T) {
	client, cleanup := setupTestConsul(t)
	defer cleanup()
	...
	// List services with name filter
	resp, err := provider.ListResources(&pb.ListResourcesRequest{
		ResourcePath: proto.String("services"),
		Filter: []*pb.Filter{
	...
	// Verify additional labels are present on all resources
	for _, res := range resp.Resources {
		for key, expectedValue := range additionalLabels {
			if gotValue, exists := res.Labels[key]; !exists {
				t.Errorf("Resource %s missing additional label %s", res.GetName(), key)
			} else if gotValue != expectedValue {
				// Check if it's the environment label which might have been set by service metadata
				if key == "environment" {
					// Service metadata takes precedence, so this is expected
					t.Logf("Resource %s has environment label %s (service metadata), additional label was %s",
						res.GetName(), gotValue, expectedValue)
				} else if gotValue != expectedValue {
					t.Errorf("Resource %s label %s = %s, want %s", res.GetName(), key, gotValue, expectedValue)
				}
			}
		}
	}

	t.Logf("Successfully verified additional labels on %d resources", len(resp.Resources))
}
