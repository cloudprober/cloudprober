...
func newWideSurfacer(labelCols map[string]string, metricCols map[string]string) *Surfacer {
	wideRows := true
	var bqCols []*configpb.BQColumn
	...
	mc := make(map[string]*configpb.BQColumn)
	for metricName, colType := range metricCols {
		mn, ct := metricName, colType
		col := &configpb.BQColumn{
			MetricName: &mn,
			ColumnName: &mn,
			ColumnType: &ct,
		}
		bqCols = append(bqCols, col)
		mc[metricName] = col
	}
	projectName := "test-project"
	bqdataset := "test-dataset"
