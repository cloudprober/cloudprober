...
// clientSpanNamed returns a pointer to the span named name, if present.
func clientSpanNamed(spans tracetest.SpanStubs, name string) *tracetest.SpanStub {
	for i := range spans {
...
// redactQueryParamsAttr returns the otelsdk.RedactQueryParamsAttrKey
// attribute's value on span, if present.
func redactQueryParamsAttr(span *tracetest.SpanStub) ([]string, bool) {
	for _, kv := range span.Attributes {
		if kv.Key == otelsdk.RedactQueryParamsAttrKey {
...
// The tracing package's exporter wrapper is the thing that actually redacts
// query params (see internal/tracing/redact_exporter.go); here we only
// verify that the probe attaches the marker attribute it reads from,
// carrying the configured param names, directly on the span it creates --
// with no shared state involved.
func TestProbeSetsRedactQueryParamsAttrOnSpan(t *testing.T) {
	exp, tp := setupTestTracing(t)
	...
	opts.ProbeConf = &configpb.ProbeConf{
		Port:                      proto.Int32(int32(port)),
		RelativeUrl:               proto.String("/?token=secret"),
		RedactQueryParamsInTraces: []string{"token"},
	}
	opts.ProberConfig = &proberconfigpb.ProberConfig{Tracing: &tracingpb.TracingConfig{}}
	...
	clientSpan := clientSpanNamed(exp.GetSpans(), "GET /")
	if !assert.NotNil(t, clientSpan, "expected a client span named %q", "GET /") {
		return
	}

	got, ok := redactQueryParamsAttr(clientSpan)
	assert.True(t, ok, "expected redact-query-params marker attribute on client span")
	assert.Equal(t, []string{"token"}, got)
}

func TestProbeOmitsRedactQueryParamsAttrWhenUnset(t *testing.T) {
	_, spans := runTracedProbe(t, &tracingpb.TracingConfig{})

	clientSpan := clientSpanNamed(spans, "GET /")
	if !assert.NotNil(t, clientSpan, "expected a client span named %q", "GET /") {
		return
	}

	_, ok := redactQueryParamsAttr(clientSpan)
	assert.False(t, ok, "did not expect redact-query-params marker attribute when unset")
}
