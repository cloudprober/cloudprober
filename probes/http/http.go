...
func (p *Probe) httpClient(target endpoint.Endpoint) *http.Client {
	...
	if p.enableTracing {
		...
		// See otelsdk.RedactQueryParamsAttrKey: this attribute is consumed
		// and stripped by internal/tracing's exporter wrapper, and only
		// added when there's something to redact.
		if keys := p.c.GetRedactQueryParamsInTraces(); len(keys) > 0 {
			spanAttrs = append(spanAttrs, otelsdk.RedactQueryParamsAttrKey.StringSlice(keys))
		}
		...
		opts := []otelhttp.Option{
			...
			otelhttp.WithSpanOptions(trace.WithAttributes(spanAttrs...)),
